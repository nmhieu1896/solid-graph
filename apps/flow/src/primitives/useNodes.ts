...
const res: any = await axios.get('https://platform-api.sens-vn.com/graph/1').then((data) => data.data.data[0].nodes);

//----------- Node And Edge-------------
export const nodes = new Nodes(res);
...
let debounce: any;
createEffect(() => {
  const newNodes = nodes.nodes().map((node) => node.get());
